...
def agregar_resultado():
    ...
    absorbancia_input = st.number_input(f'Ingresa el valor de absorbancia {len(absorbancias_input) + 1}:', 
                                        min_value=0.001, max_value=10.0, step=0.001, value=0.275, format="%.3f")
...
# *** Extend calibration line ***
# We will extrapolate the curve beyond the last calibration point
extended_concentration_cal = np.linspace(0, max(max(concentraciones), max(concentracion_cal)) * 1.2, 100)
extended_absorbancia_cal = interp_func(extended_concentration_cal)
...
# *** Plot the extended blue line ***
ax.plot(extended_concentration_cal, extended_absorbancia_cal, label='Curva de Calibración (Calibrador)', color='blue')
